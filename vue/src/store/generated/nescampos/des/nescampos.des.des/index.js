    ...
    actions: {
        ...
        async QueryElection({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryElection', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryElectionAll({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).queryElectionAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    for (let prop of Object.keys(next_values)) {
                        if (Array.isArray(next_values[prop])) {
                            value[prop] = [...value[prop], ...next_values[prop]];
                        }
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryElectionAll', payload: { options: { all }, params: { ...key }, query } });
