...
func (k msgServer) CreateVote(goCtx context.Context, msg *types.MsgCreateVote) (*types.MsgCreateVoteResponse, error) {
    ...
    for _, existingVote := range voteList {
        // Check if the account has already voted on this ElectionID
        if existingVote.Creator == msg.Creator && existingVote.ElectionID == msg.ElectionID {
              // Return an error when a vote has been cast by this account on this ElectionID
